...
@Component
@Scope("session")
public class ConsolidatedMarkingRnlAction implements Serializable{
	private Logger log = Logger.getLogger(ConsolidatedMarkingRnlAction.class);
	...
	public void markAsReceived(){
		try{
			Date now = new Date();
			List<LicPolicyDtls> list = licBrnhHubPicPodDtlsService.findPolicyDtlsByPodId(podId);
			...
			licMarkingToQcDtls.setLicPolicyDtlses(list);
			
			for(LicPolicyDtls obj:list){
			...
			log.info("ConsolidatedMarkingRnlAction markAsReceived Error : ", e);
		}
	}
	...
	public String onLoad(){
		try{
			...
			receivedDate = null;
			List<Object> list = licBrnhHubPicPodDtlsService.findDistinctPodDtlsForConsldMarkingForRnl(loginAction.findHubForProcess("RNL"));
			...
			return "/licHubRenewalActivity/consolidatedMarkingRnl.xhtml";
		}catch(Exception e){
			log.info("ConsolidatedMarkingRnlAction onLoad Error : ", e);
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
