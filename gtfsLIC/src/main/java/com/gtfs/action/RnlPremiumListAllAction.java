...
@Component
@Scope("session")
public class RnlPremiumListAllAction implements Serializable{
	private Logger log = Logger.getLogger(RnlPremiumListAllAction.class);
	...
	private List<LicHubMst> licHubMstList = new ArrayList<LicHubMst>();
	...
	public void loadingPicForHub(){
		...
		licHubMstList = licHubMstService.findActiveHubMst();
	...
	public String onLoad(){
		refresh();
		return "/licHubRenewalActivity/rnlPremiumListAll.xhtml";
	}
	...
	public void search(){
		try{
			...
			licHubMsts.add(licHubMstService.findById(hubId));
			...
			List<Object> list = licPremiumListService.findPolicyDtlsForPremList(licHubMsts, payMode, healthReq, payFromDate, payToDate);


			if(list == null || list.size() == 0 || list.contains(null)){
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
	                    "Eror : ", "Nothing Record(s) Found"));
				return;
			}
			...
			log.info("RnlPremiumListAllAction Search Error : ", e);
		}
	}
	...
	public void save(){
		try{
			...
			Date now = new Date();
			//List<LicPremPolicyMapping> licPrePolicyMappings = new ArrayList<LicPremPolicyMapping>();
			...
			log.info("RnlPremiumListAllAction Save Error : ", e);
		}
	}
	...
	public List<LicHubMst> getLicHubMstList() {
		return licHubMstList;
	}
	public void setLicHubMstList(List<LicHubMst> licHubMstList) {
		this.licHubMstList = licHubMstList;
	}
