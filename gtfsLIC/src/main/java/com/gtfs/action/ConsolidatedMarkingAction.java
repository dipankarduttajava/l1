...
@Component
@Scope("session")
public class ConsolidatedMarkingAction implements Serializable{
	private Logger log = Logger.getLogger(ConsolidatedMarkingAction.class);
	...
	public void markAsReceived(){
		try{
			Date now = new Date();
			List<LicOblApplicationMst> list = licBrnhHubPicPodDtlsService.findApplicationByPodId(podId);
			...
			licMarkingToQcDtls.setLicOblApplicationMsts(list);
			...
			log.info("ConsolidatedMarkingAction markAsReceived Error : ", e);
		}
	}
	...
	public String onLoad(){
		try{			
			...
			receivedDate = null;
			List<Object> list = licBrnhHubPicPodDtlsService.findDistinctPodDtlsForConsldMarking(loginAction.findHubForProcess("OBL"));
			...
			return "/licHubActivity/consolidatedMarking.xhtml";
		}catch(Exception e){
			log.info("ConsolidatedMarkingAction onLoad Error : ", e);
			return null;
		}
	}
